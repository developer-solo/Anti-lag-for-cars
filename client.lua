RegisterNetEvent("solo:kuracstopuca")
...
AddEventHandler("solo:kuracstopuca", function()
	if antilag == false then
		antilag = true
		Notif("~c~Upalio si ~b~pucanje!")
	else
		antilag = false
		Notif("~c~Ugasio si ~r~pucanje!")
	end
end)
...
Citizen.CreateThread(function()
	while true do
		if activated == true then
			DrawHudText("Puca", {0, 255, 85,255},0.92,0.88,0.7,0.7,6)
		end
		if AntilagDisplay == true then
			DrawHudText("Pucanje", {0, 255, 85,255},0.92,0.88,0.7,0.7,6)
		end
	  Wait(0)
	end
end)
